...
function App() {
    ...
    const addToCart = (product: ProductType) => {
        ...
        const isAdded = cartItems.filter(el => el.id === product.id)
        if (isAdded.length === 0) {
    ...
    const onEnterDown = (event: KeyboardEvent<HTMLInputElement>) => {
        const filteredItems = initialData.filter(item => item.title.toLowerCase().includes(searchValue.toLowerCase()))
    ...
    return (
        <div className="App">
            ...
            <ProductList productsList={copyOfProducts} addToCart={addToCart} setFilter={setFilter}/>
        </div>
    );
